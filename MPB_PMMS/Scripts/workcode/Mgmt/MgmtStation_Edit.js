...
$(document).ready(function () {
    ...
    $("#ST_CODE").on("blur", function (event) { check_key_duplicate(); });
...
function check_key_duplicate() {
    ...
    var ST_CODE = $("#ST_CODE");
    var ST_CODE_value = ST_CODE.val();
    if (ST_CODE_value == null) { ST_CODE_value = ""; }
    if ($("#mode").val() == "M" && ($("#chk_Value").val() == ST_CODE_value)) { return false; }
    if (ST_CODE_value != "") {   //all keys not null
        $.ajax({
            url: "AjaxCheckKey",
            data: {
                "ST_CODE": ST_CODE_value
            },
            ...
            success: function (responseData, textStatus, xhr) {//responseData may equal to xhr.responseText
                ...
                if (xmlhttpResponseObj != null && xmlhttpResponseObj.ToTalRow > 0) {//query成功
                    ...
                    if (Data[0].KeyCount <= 0) {
                        ...
                        ST_CODE.vs_showDivAlert({ alertMassage: "" + ST_CODE.attr("title") + "(" + ST_CODE_value + ")已存在<br>,不能使用!" });
                        ST_CODE.val("" + ST_CODE.prop("defaultValue"));
                        ST_CODE.delay(9, function () { $(this).trigger("focus"); });
                    }
                } else {
                    ST_CODE.vs_showDivAlert({ alertMassage: "資料有誤(ToTalRow=0),不能確定" + ST_CODE.attr("title") + "(" + ST_CODE_value + ")是否已存在!" });
                    ST_CODE.val("" + ST_CODE.prop("defaultValue"));
                    ST_CODE.delay(9, function () { $(this).trigger("focus"); });
                }
            },
            error: function (xhr, textStatus, errorThrown) {
                ...
                ST_CODE.vs_showDivAlert({ alertMassage: "網路有誤(" + textStatus + "),不能確定" + ST_CODE.attr("title") + "(" + ST_CODE_value + ")是否已存在!" });
                ST_CODE.val("" + ST_CODE.prop("defaultValue"));
                ST_CODE.delay(9, function () { $(this).trigger("focus"); });
            },
...
function doSave() {
    ...
    $('form:first').vs_doSubmit({ actionPage: "MgmtStation_Save", targetWin: "iframeHide", postMethod: "post" });
}
