...
$(document).ready(function () {
    ...
    if (isEditMode) {
        $("#CR_DATE").vs_initialForm({ D: 1 });
        $("#CR_TITLE").vs_initialForm({ D: 1 });
        $("#CR_CONTENT").vs_initialForm({ D: 1 });
        $("#NAME").vs_initialForm({ D: 1 });
        $("#PHONE").vs_initialForm({ D: 1 });
        $("#EMAIL").vs_initialForm({ D: 1 });
    } else {
    }
...
function doSave() {
    ...
    $('form:first').vs_doSubmit({ actionPage: "SysConsRec_Save", targetWin: "iframeHide", postMethod: "post" });
}
